...
class TestWin(QWidget):
    def __init__(self):
        ...
        super().__init__()
        self.first_button_start = False
        self.second_button_start = False
        self.final_button_start = False
    ...
    def timer3Event(self):
        ...
        if time.toString("hh:mm:ss") == "00:00:00":
            self.timer.stop()
            self.final_button_start = True
            self.connects()


    def connects(self):
        self.btn_test1.clicked.connect(self.timer_test)
        if self.first_button_start == True:
            self.btn_test2.clicked.connect(self.timer_sits)
        if self.second_button_start == True:
            self.btn_test3.clicked.connect(self.timer_final)
        if self.final_button_start == True:
